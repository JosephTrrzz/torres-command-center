  ...
  return <section className="detail-card profile-editor">
    ...
    {open && <form className="profile-form" onSubmit={save}>
      <div className="form-grid">
        ...
        <label>Website<input type="url" value={form.website} onChange={(event) => change("website", event.target.value)} required /></label>
        <label>Email<input type="email" value={form.email ?? ""} onChange={(event) => change("email", event.target.value)} /></label>
        <label>Phone number<input type="tel" value={form.phone ?? ""} onChange={(event) => change("phone", event.target.value)} /></label>
      </div>
