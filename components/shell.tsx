...
export function Shell({ children, active }: { children: React.ReactNode; active: string }) {
  ...
  return <div className="app-shell">
    ...
    <main className="main">
      ...
      <div className="topbar"><span className="breadcrumb">Torres &amp; Co. <b>/</b> {active}</span><div className="top-actions"><div className="header-menu"><button type="button" className="round-button notification-trigger" onClick={() => setNotice(!notice)} aria-label={`Notifications${unreadCount ? `, ${unreadCount} unread` : ""}`} aria-expanded={notice}><span className="notification-bell" aria-hidden="true" />{unreadCount > 0 && <b className="notification-count">{unreadCount}</b>}</button>{notice && <div className="menu-popover notification-popover" role="dialog" aria-label="Workspace notifications"><div className="notification-heading"><div><strong>Notifications</strong><small>{unreadCount ? `${unreadCount} unread` : "All caught up"}</small></div>{unreadCount > 0 && <button type="button" className="mark-read" onClick={markAllRead}>Mark all read</button>}</div><div className="notification-list">{workspaceNotifications.map((item) => <div className={`notification-item ${readNotifications.includes(item.id) ? "is-read" : ""}`} key={item.id}><span className={`notification-dot ${item.tone}`} aria-hidden="true" /><div><strong>{item.title}</strong><p>{item.detail}</p><small>{item.time}</small></div></div>)}</div><Link className="notification-footer" href="/settings/#admin-console" onClick={() => setNotice(false)}>Review workspace settings <span>→</span></Link></div>}</div><div className="header-menu"><button type="button" className="top-avatar" onClick={() => setProfile(!profile)} aria-label="Open profile menu" aria-expanded={profile}>{avatarImage ? <img src={avatarImage} alt="" /> : avatar}</button>{profile && <div className="menu-popover profile-popover"><strong>{displayName}</strong><small>{accessLabel}</small><button type="button" onClick={logout}>Log out</button></div>}</div></div></div>
