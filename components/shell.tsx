...
export function Shell({ children, active }: { children: React.ReactNode; active: string }) {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${open ? "open" : ""}`}>
      ...
      <nav aria-label="Main navigation">{nav.map((item) => <Link className={active === item.label ? "active" : ""} href={item.href} key={item.label}><span className="nav-icon">{item.label === "Overview" ? "◈" : item.label === "Clients" ? "◎" : item.label === "Portal" ? "↗" : item.label === "Integrations" ? "✦" : item.label === "Reports" ? "▤" : "⚙"}</span>{item.label}{item.label === "Clients" && <em>3</em>}</Link>)}</nav>
