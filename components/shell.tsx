...
export function Shell({ children, active }: { children: React.ReactNode; active: string }) {
  ...
  const shell = <div className="app-shell" data-shell-variant={effectiveRole === "customer" ? "client" : "internal"}>
    ...
    {open && <button type="button" className="shell-overlay" aria-label="Close navigation" onClick={() => { setOpen(false); setWorkspaceOpen(false); }} />}
    <aside className={`sidebar ${open ? "open" : ""}`} id="workspace-navigation">
      ...
      <nav aria-label={effectiveRole === "customer" ? "Client workspace navigation" : "Internal workspace navigation"}>{nav.map((item) => <Link onClick={() => { setOpen(false); setWorkspaceOpen(false); }} className={activeNavigationLabel === item.label ? "active" : ""} aria-current={activeNavigationLabel === item.label ? "page" : undefined} href={item.href} key={item.label}><span className="nav-icon"><AppIcon name={NAV_ICONS[item.label] ?? "settings"} /></span>{effectiveRole === "customer" ? CLIENT_NAV_LABELS[item.label] ?? item.label : item.label}{item.label === "Clients" && effectiveRole !== "customer" && clients.length > 0 && <em aria-label={`${clients.length} clients`}>{clients.length}</em>}</Link>)}</nav>
    ...
    <main className="main">
      <header className="mobile-header"><button onClick={() => { setOpen(!open); if (open) setWorkspaceOpen(false); }} aria-label={open ? "Close menu" : "Open menu"} aria-controls="workspace-navigation" aria-expanded={open}>☰</button><span className="brand-mark">T</span><strong>{effectiveRole === "customer" ? "Private Office" : "Torres & Co."}</strong><button className="mobile-logout" onClick={logout}>Log out</button></header>
