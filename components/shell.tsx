...
export function Shell({ children, active }: { children: React.ReactNode; active: string }) {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${open ? "open" : ""}`}>
      ...
      <nav aria-label="Main navigation">{nav.map((item) => <Link onClick={() => setOpen(false)} className={active === item.label ? "active" : ""} aria-current={active === item.label ? "page" : undefined} href={item.href} key={item.label}><span className="nav-icon" aria-hidden="true">{item.label === "Today" ? "☼" : item.label === "Overview" ? "◈" : item.label === "Clients" ? "◎" : item.label === "CRM" ? "◉" : item.label === "Projects" ? "◇" : item.label === "Operations" ? "▦" : item.label === "Onboarding" ? "✓" : item.label === "Portal" || item.label === "My account" ? "↗" : item.label === "Integrations" ? "✦" : item.label === "Reports" ? "▤" : "⚙"}</span>{item.label}{item.label === "Clients" && effectiveRole !== "customer" && clients.length > 0 && <em aria-label={`${clients.length} clients`}>{clients.length}</em>}</Link>)}</nav>
