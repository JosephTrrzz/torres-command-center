...
export function Shell({ children, active }: { children: React.ReactNode; active: string }) {
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${open ? "open" : ""}`}>
      ...
      <div className="sidebar-bottom"><div className="profile"><span className="avatar">{avatar}</span><div><strong>{displayName}</strong><small>{accessLabel}</small></div><button className="logout-button" onClick={logout}>Log out</button></div></div>
    </aside>
    <main className="main">
