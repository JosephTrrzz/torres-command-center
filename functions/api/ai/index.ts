...
async function authorizedEvidence(context: AuthContext, url: string, serviceKey: string) {
  ...
  const conversationVisibility = scope.clientUser ? "&client_visible=eq.true" : "";
  const requests: Array<Promise<TorresAiEvidence[]>> = [
    ...
    restCount(url, serviceKey, `conversations?organization_id=in.(${organizationFilter})${conversationVisibility}&archived_at=is.null&status=in.(open,pending)&select=id`).then((count) => evidenceItem("notification", "workspace-open-conversations", "Inbox workload", `Exact current total: ${count} open or pending conversation${count === 1 ? "" : "s"} ${count === 1 ? "is" : "are"} in the authorized inbox.`, "/inbox/", observedAt)).then((item) => item ? [item] : []),
    ...(scope.clientIds.length ? [rest<{ id: string; name: string; industry: string; location: string }>(url, serviceKey, `clients?id=in.(${clientFilter})&select=id,name,industry,location&order=name.asc&limit=4`).then((rows) => rows.flatMap((row) => evidenceItem("client", row.id, row.name, `${row.name} is a ${clean(row.industry, 100) || "client"} account${row.location ? ` in ${clean(row.location, 180)}` : ""}.`, scope.clientUser ? "/portal/" : `/clients/?client=${row.id}`, null) || []))] : []),
    rest<{ id: string; name: string; status: string; progress_percent: number; target_date: string | null; updated_at: string }>(url, serviceKey, `client_projects?organization_id=in.(${organizationFilter})&select=id,name,status,progress_percent,target_date,updated_at&order=updated_at.desc&limit=5`).then((rows) => rows.flatMap((row) => evidenceItem("project", row.id, row.name, `Project status: ${row.status}; progress: ${row.progress_percent}%; target: ${row.target_date || "not set"}.`, `/projects/`, row.updated_at) || [])),
    rest<{ id: string; title: string; status: string; priority: string; scheduled_start: string | null; updated_at: string }>(url, serviceKey, `service_jobs?organization_id=in.(${organizationFilter})${serviceVisibility}&select=id,title,status,priority,scheduled_start,updated_at&order=updated_at.desc&limit=5`).then((rows) => rows.flatMap((row) => evidenceItem("service_job", row.id, row.title, `Service status: ${row.status}; priority: ${row.priority}; scheduled: ${row.scheduled_start || "not scheduled"}.`, `/operations/`, row.updated_at) || [])),
    rest<{ id: string; report_type: string; period_start: string; period_end: string; created_at: string }>(url, serviceKey, `report_snapshots?organization_id=in.(${organizationFilter})&select=id,report_type,period_start,period_end,created_at&order=created_at.desc&limit=4`).then((rows) => rows.flatMap((row) => evidenceItem("report_snapshot", row.id, `${row.report_type} report`, `Verified report snapshot covers ${row.period_start} through ${row.period_end}.`, `/reports/`, row.created_at) || [])),
    rest<{ id: string; title: string; body: string; href: string | null; created_at: string }>(url, serviceKey, `notifications?user_id=eq.${encodeURIComponent(context.userId)}&select=id,title,body,href,created_at&order=created_at.desc&limit=2`).then((rows) => rows.flatMap((row) => evidenceItem("notification", row.id, row.title, row.body, notificationHref(row.href, scope.clientUser), row.created_at) || [])),
  ];
  if (!scope.clientUser && scope.clientIds.length) {
    requests.push(
      ...
      restCount(url, serviceKey, `crm_appointments?client_id=in.(${clientFilter})&status=eq.scheduled&starts_at=gte.${encodeURIComponent(observedAt)}&select=id`).then((count) => evidenceItem("crm_lead", "workspace-upcoming-appointments", "Upcoming appointments", `Exact current total: ${count} scheduled appointment${count === 1 ? "" : "s"} ${count === 1 ? "is" : "are"} upcoming.`, "/schedule/", observedAt)).then((item) => item ? [item] : []),
      restCount(url, serviceKey, `crm_tasks?client_id=in.(${clientFilter})&status=in.(open,in_progress)&select=id`).then((count) => evidenceItem("crm_lead", "workspace-open-tasks", "Open CRM tasks", `Exact current total: ${count} CRM follow-up task${count === 1 ? "" : "s"} ${count === 1 ? "is" : "are"} open or in progress.`, "/crm/", observedAt)).then((item) => item ? [item] : []),
      restCount(url, serviceKey, `integration_connections?organization_id=in.(${organizationFilter})&status=eq.connected&select=id`).then((count) => evidenceItem("notification", "workspace-connected-integrations", "Connected integrations", `Exact current total: ${count} integration${count === 1 ? "" : "s"} ${count === 1 ? "is" : "are"} connected.`, "/integrations/", observedAt)).then((item) => item ? [item] : []),
      restCount(url, serviceKey, `integration_connections?organization_id=in.(${organizationFilter})&status=in.(degraded,action_required)&select=id`).then((count) => evidenceItem("notification", "workspace-integrations-needing-attention", "Integrations needing attention", `Exact current total: ${count} integration${count === 1 ? "" : "s"} ${count === 1 ? "needs" : "need"} attention.`, "/integrations/", observedAt)).then((item) => item ? [item] : []),
      ...
      rest<{ id: string; title: string; starts_at: string; ends_at: string; timezone: string; location: string; updated_at: string }>(url, serviceKey, `crm_appointments?client_id=in.(${clientFilter})&status=eq.scheduled&starts_at=gte.${encodeURIComponent(observedAt)}&select=id,title,starts_at,ends_at,timezone,location,updated_at&order=starts_at.asc&limit=3`).then((rows) => rows.flatMap((row) => evidenceItem("crm_lead", `appointment-${row.id}`, row.title, `Scheduled from ${row.starts_at} to ${row.ends_at} in ${row.timezone}${row.location ? ` at ${clean(row.location, 180)}` : ""}.`, "/schedule/", row.updated_at) || [])),
      rest<{ id: string; title: string; due_at: string | null; priority: string; status: string; updated_at: string }>(url, serviceKey, `crm_tasks?client_id=in.(${clientFilter})&status=in.(open,in_progress)&select=id,title,due_at,priority,status,updated_at&order=due_at.asc.nullslast&limit=3`).then((rows) => rows.flatMap((row) => evidenceItem("crm_lead", `task-${row.id}`, row.title, `Task status: ${row.status}; priority: ${row.priority}; due: ${row.due_at || "not set"}.`, "/crm/", row.updated_at) || [])),
    );
  }
...
export const onRequestPost = async ({ request, env }: { request: Request; env: Env }) => {
  ...
  try {
    ...
    if (!response.ok || !agentResult || !answer || (agentResult.grounded && !citationIds.length) || (!agentResult.grounded && citationIds.length > 0)) {
