...
export const onRequestPost = async ({ request, env }: { request: Request; env: Env }) => {
  ...
  const insertResponse = await fetch(`${url}/rest/v1/crm_leads?on_conflict=external_provider,external_submission_id`, {
    ...
    body: JSON.stringify({
      ...
      source: "website",
      status: "new",
      external_provider: "torres_website",
  ...
  await Promise.allSettled([
    ...
    fetch(`${url}/rest/v1/audit_events`, {
      method: "POST",
      headers: serviceHeaders(serviceKey, "return=minimal"),
      body: JSON.stringify({ organization_id: client.organization_id, action: "crm.lead.created", entity_type: "crm_lead", entity_id: leadId, source: "torres_website", metadata: eventMetadata }),
    }),
