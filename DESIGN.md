...
Torres AI uses a two-part evidence desk: a private conversation rail and a restrained answer ledger. Every factual assistant answer must show server-verified source links and a confidence label; model-created links are never rendered. The trust bar always states that the surface is private, tenant-isolated, and read-only. Prompts render as plain text, while answers use complete sentences, short paragraphs, and semantic lists for multiple findings. Workspace records are treated as untrusted evidence rather than instructions. Unsupported questions persist as a low-confidence “not integrated” response without citations and must never be matched to unrelated evidence. A submitted prompt appears immediately in a muted pending state, followed in the same ledger by a compact Torres AI review state; neither reports fake stages or progress. New ledger entries use the standard 220ms three-pixel reveal and become static under reduced motion. On narrow screens the thread rail becomes a horizontal list, the composer stacks, and no content may overflow the viewport.
