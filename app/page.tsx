...
export default function DashboardPage() {
  ...
  return <Shell active="Overview">
    <div className="page-heading"><div><p className="eyebrow">{dateLabel}</p><h1>{greeting}, Joseph.</h1><p className="lede">Here’s what’s happening across your client portfolio.</p></div><Link className="button button-dark" href="/clients">View all clients <span>→</span></Link></div>
    ...
    <section className="activity-panel"><div className="section-heading"><div><p className="eyebrow">Workspace flow</p><h2>Next actions</h2></div></div><div className="action-grid"><Link href="/clients/"><strong>1. Add or invite a client</strong><span>Create the account record and send an activation link.</span><b>Open Clients →</b></Link><Link href="/integrations/"><strong>2. Map reporting properties</strong><span>Choose the matching Google resources for each client.</span><b>Open Integrations →</b></Link><Link href="/reports/"><strong>3. Review before sharing</strong><span>Preview live metrics, then print or download the report.</span><b>Open Reports →</b></Link></div></section>
  </Shell>;
}
