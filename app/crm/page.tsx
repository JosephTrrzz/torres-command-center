  ...
  return <Shell active="CRM">
    ...
    {loading ? <LoadingRegion active label="Loading agency pipeline" variant="crm" /> : !snapshot ? <section className="empty-state"><h2>CRM is unavailable</h2><p>The agency pipeline could not be loaded.</p></section> : <>
      ...
      {snapshot.canManage && <details className="crm-composer"><summary><span><b>Capture a lead</b><small>Add a real inquiry and assign the first response.</small></span><i>＋</i></summary><form onSubmit={createLead}>
        ...
        <p className="crm-form-note">A name and either email or phone are required.</p><button className="button button-dark" disabled={busy === "new-lead"}>{busy === "new-lead" ? "Saving…" : "Add lead →︎"}</button>
      </form></details>}
          ...
          {snapshot.canManage && selectedLead.email && <section className="crm-email-composer">
            ...
            <form key={selectedLead.id} onSubmit={sendLeadEmail}>
              ...
              <div className="crm-email-composer-actions">
                ...
                <button className="button button-dark" disabled={busy === "lead-email"}>{busy === "lead-email" ? "Sending…" : "Send email →︎"}</button>
              </div>
            </form>
          </section>}

          {snapshot.canManage && <form className="crm-control-row" key={selectedLead.id} onSubmit={updateLead}><label>Pipeline stage<select name="status" defaultValue={selectedLead.status}>{LEAD_STATUSES.map((status) => <option key={status} value={status}>{labelCrmValue(status)}</option>)}</select></label><label>Owner<select name="assignedTo" defaultValue={selectedLead.assigned_to || ""}><option value="">Unassigned</option>{snapshot.team.map((member) => <option key={member.id} value={member.id}>{member.name}</option>)}</select></label><button className="button button-dark" disabled={busy === "lead-update"}>{busy === "lead-update" ? "Saving…" : "Update lead →︎"}</button></form>}
