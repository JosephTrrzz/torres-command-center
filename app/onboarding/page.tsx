...
export default function OnboardingPage() {
  ...
  return <Shell active="Onboarding">
    {adminMode && <div className="portal-preview-banner"><strong>Agency editing mode</strong><span>You are completing onboarding for this client. Every saved change is recorded in the workspace audit history.</span><Link href={`/clients/detail/?id=${encodeURIComponent(snapshot.clientId)}`}>Exit onboarding →</Link></div>}
    ...
    <div className="onboarding-shell">
      ...
      <section className="onboarding-panel">
        ...
        {activeStep === "review" && <div className="onboarding-review-grid"><article><span>Business</span><strong>{data.business.displayName}</strong><p>{data.business.vertical}{data.business.website ? ` · ${data.business.website}` : ""}</p><button type="button" onClick={() => setActiveStep("business")}>Edit business →</button></article><article><span>Primary location</span><strong>{data.location.name}</strong><p>{[data.location.streetAddress, data.location.city, data.location.region, data.location.postalCode].filter(Boolean).join(", ")}</p><button type="button" onClick={() => setActiveStep("location")}>Edit location →</button></article><article><span>Services</span><strong>{snapshot.skippedSteps.includes("services") ? "Skipped for now" : `${data.services.filter((item) => item.name).length} added`}</strong><p>{data.services.filter((item) => item.name).map((item) => item.name).join(", ") || "No services added"}</p><button type="button" onClick={() => setActiveStep("services")}>Edit services →</button></article><article><span>Goals</span><strong>{snapshot.skippedSteps.includes("goals") ? "Skipped for now" : `${data.goals.filter((item) => item.title).length} added`}</strong><p>{data.goals.filter((item) => item.title).map((item) => item.title).join(", ") || "No goals added"}</p><button type="button" onClick={() => setActiveStep("goals")}>Edit goals →</button></article></div>}

        {error && <p className="login-notice error" role="alert">{error}</p>}
        {message && <p className="login-notice success" role="status">{message}</p>}
        <div className="onboarding-actions"><button className="button button-secondary" type="button" disabled={activeStep === "business" || busy} onClick={() => setActiveStep(ONBOARDING_STEPS[Math.max(0, stepIndex(activeStep) - 1)])}>Back</button><div>{(activeStep === "services" || activeStep === "goals") && <button className="button button-light" type="button" disabled={busy} onClick={() => void saveStep({ skipped: true })}>Skip for now</button>}<button className="button button-dark" type="button" disabled={busy} onClick={() => void saveStep(activeStep === "review" ? { complete: true } : {})}>{busy ? "Saving…" : activeStep === "review" ? snapshot.status === "complete" ? "Save reviewed profile" : "Complete onboarding" : "Save and continue"} <span>→</span></button></div></div>
      </section>
    </div>
  </Shell>;
}
