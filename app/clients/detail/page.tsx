...
export default function ClientDetailPage() {
  ...
  if (!id || !client) {
    return <Shell active="Clients"><div className="detail-card"><h1>{id ? "Client not found" : "Loading account…"}</h1><Link className="text-link" href="/clients/">←︎ Back to clients</Link></div></Shell>;
  }

  return <Shell active="Clients">
    <Link className="back-link" href="/clients/">←︎ Back to clients</Link>
    ...
    <div className="detail-layout">
      <section className="detail-card"><p className="eyebrow">Business information</p><h2>Account overview</h2><p>{client.overview}</p><div className="contact-list">{client.email && <a href={`mailto:${client.email}`}>✉ {client.email}</a>}{client.phone && <a href={`tel:${client.phone}`}>☎ {client.phone}</a>}{client.website && <a href={client.website.startsWith("http") ? client.website : `https://${client.website}`} target="_blank" rel="noreferrer">↗︎ {client.website}</a>}</div></section>
    ...
    <section className="detail-card integration-preview">
      <div className="section-heading">
        ...
        <Link className="button button-dark" href={`/integrations/?client=${encodeURIComponent(client.id)}`}>
          Manage connections <span>→︎</span>
        </Link>
      </div>
