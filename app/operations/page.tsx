  ...
  return <Shell active="Operations">
    ...
    {loading ? <section className="operations-loading"><strong>Loading operations…</strong><span>Checking customer, job, schedule, and approval records.</span></section> : !snapshot ? <section className="empty-state"><h2>Operations workspace unavailable</h2><p>Choose a client or apply the Phase 3 operations migration.</p></section> : <>
      ...
      <section className="customer-360 operations-panel">
        <div className="operations-section-heading"><div><p className="eyebrow">Customer 360</p><h2>{snapshot.client.name}</h2><p>{[snapshot.client.industry, snapshot.client.location].filter(Boolean).join(" · ") || "Customer profile"}</p></div>{snapshot.client.website && <a href={snapshot.client.website} target="_blank" rel="noreferrer">Open website ↗</a>}</div>
        <div className="customer-360-grid">
          <article><span>Contacts</span><strong>{snapshot.contacts.length}</strong>{snapshot.contacts.length ? snapshot.contacts.map((contact) => <div className="customer-row" key={contact.id}><b>{contact.name}</b><small>{[contact.role, contact.email, contact.phone].filter(Boolean).join(" · ")}</small></div>) : <small>No contacts added yet.</small>}</article>
          <article><span>Locations</span><strong>{snapshot.locations.length}</strong>{snapshot.locations.length ? snapshot.locations.map((location) => <div className="customer-row" key={location.id}><b>{location.label}{location.is_primary ? " · Primary" : ""}</b><small>{addressLabel(location)}</small></div>) : <small>No service locations added yet.</small>}</article>
          <article><span>Pipeline handoff</span><strong>{snapshot.leads.length}</strong><small>Won leads ready for delivery</small><span className="operations-mini-stat">{snapshot.projects.length} linked projects</span></article>
        </div>
        {snapshot.canManage && <details className="operations-composer"><summary>Add customer location <span>＋</span></summary><form className="operations-form" onSubmit={createLocation}><label>Location name<input required value={locationForm.label} onChange={(event) => setLocationForm({ ...locationForm, label: event.target.value })} placeholder="Main office" /></label><label>Street address<input value={locationForm.addressLine1} onChange={(event) => setLocationForm({ ...locationForm, addressLine1: event.target.value })} /></label><label>City<input value={locationForm.city} onChange={(event) => setLocationForm({ ...locationForm, city: event.target.value })} /></label><label>State / region<input value={locationForm.region} onChange={(event) => setLocationForm({ ...locationForm, region: event.target.value })} /></label><label>Postal code<input value={locationForm.postalCode} onChange={(event) => setLocationForm({ ...locationForm, postalCode: event.target.value })} /></label><label>Country<input maxLength={2} value={locationForm.country} onChange={(event) => setLocationForm({ ...locationForm, country: event.target.value.toUpperCase() })} /></label><label className="operations-form-wide">Access or service notes<textarea value={locationForm.accessNotes} onChange={(event) => setLocationForm({ ...locationForm, accessNotes: event.target.value })} /></label><label className="operations-check"><input type="checkbox" checked={locationForm.isPrimary} onChange={(event) => setLocationForm({ ...locationForm, isPrimary: event.target.checked })} /> Primary location</label><button className="button button-dark" disabled={busy === "location"}>Save location →</button></form></details>}
      </section>

      {snapshot.canManage && <details className="operations-composer operations-job-composer"><summary>Create a service job <span>＋</span></summary><form className="operations-form" onSubmit={createJob}><label>Job title<input required value={jobForm.title} onChange={(event) => setJobForm({ ...jobForm, title: event.target.value })} placeholder="Website support and optimization" /></label><label>Won lead<select value={jobForm.leadId} onChange={(event) => setJobForm({ ...jobForm, leadId: event.target.value })}><option value="">No lead handoff</option>{snapshot.leads.map((lead) => <option value={lead.id} key={lead.id}>{lead.full_name} · {lead.service_interest}</option>)}</select></label><label>Project<select value={jobForm.projectId} onChange={(event) => setJobForm({ ...jobForm, projectId: event.target.value })}><option value="">No linked project</option>{snapshot.projects.map((project) => <option value={project.id} key={project.id}>{project.name}</option>)}</select></label><label>Priority<select value={jobForm.priority} onChange={(event) => setJobForm({ ...jobForm, priority: event.target.value })}>{JOB_PRIORITIES.map((priority) => <option value={priority} key={priority}>{labelOperationsValue(priority)}</option>)}</select></label><label>Assignee<select value={jobForm.assignedTo} onChange={(event) => setJobForm({ ...jobForm, assignedTo: event.target.value })}><option value="">Unassigned</option>{snapshot.team.map((member) => <option value={member.id} key={member.id}>{member.name}</option>)}</select></label><label>Service location<select value={jobForm.locationId} onChange={(event) => setJobForm({ ...jobForm, locationId: event.target.value })}><option value="">No location</option>{snapshot.locations.map((location) => <option value={location.id} key={location.id}>{location.label}</option>)}</select></label><label>Starts<input type="datetime-local" value={jobForm.scheduledStart} onChange={(event) => setJobForm({ ...jobForm, scheduledStart: event.target.value })} /></label><label>Ends<input type="datetime-local" value={jobForm.scheduledEnd} onChange={(event) => setJobForm({ ...jobForm, scheduledEnd: event.target.value })} /></label><label className="operations-form-wide">Scope and service notes<textarea value={jobForm.description} onChange={(event) => setJobForm({ ...jobForm, description: event.target.value })} /></label><label className="operations-check"><input type="checkbox" checked={jobForm.clientVisible} onChange={(event) => setJobForm({ ...jobForm, clientVisible: event.target.checked })} /> Show this job to the client</label><button className="button button-dark" disabled={busy === "create-job"}>{busy === "create-job" ? "Creating…" : "Create service job →"}</button></form></details>}
