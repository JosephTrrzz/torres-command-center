...
| Torres AI | Phase 7 live foundation | Read-only tenant retrieval, exact server-calculated workspace totals, private user-owned conversations, current-evidence follow-ups, verified citations, one-pass structured-output recovery, atomic persistence, database-serialized rate limits, replay protection, privacy-minimized telemetry, and the private `torres-ai` Cloudflare Agent are implemented. A signed-in production prompt completes, persists, survives reload, and displays a verified source; deployment of the latest reasoning update and a separate two-user/two-organization isolation smoke test remain. |
