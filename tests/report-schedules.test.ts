...
const migration = readFileSync(join(process.cwd(), "supabase", "reporting.sql"), "utf8");
...
describe("scheduled report dates", () => {
  ...
  it("checks run-only schedule fields only inside the run-table branch", () => {
    expect(migration).toContain("if tg_table_name = 'report_schedule_runs' then");
    expect(migration).not.toContain("if tg_table_name = 'report_schedule_runs' and not exists");
  });
});
