...
| `event_outbox` | Durable background work for providers, notifications, analytics, and automations. | Server workers only |
| `ai_threads` / `ai_messages` | Retired Torres AI history retained without application-role access. | Controlled operator disposition only |
| `ai_citations` | Retired citation history retained without application-role access. | Controlled operator disposition only |
| `ai_approvals` | Retired approval history retained without application-role access. | Controlled operator disposition only |
| `ai_usage_events` | Retired privacy-minimized telemetry retained without application-role access. | Controlled operator disposition only |
...
14. On databases where Torres AI was previously installed, apply [`retire_torres_ai.sql`](./retire_torres_ai.sql) to revoke application access while preserving historical records.
