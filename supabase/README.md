...
| `provider_metric_observations` | Daily, tenant-scoped GA4 and Search Console measurements normalized into one provider-independent shape. Contains no OAuth credentials or raw provider payloads. | Integrations sync; read in Reports |
...
| `ai_threads` / `ai_messages` | Private, user-owned Torres AI conversations scoped to one organization. | Torres AI |
| `ai_citations` | Server-verified source links attached to an AI answer. | Torres AI (server-managed) |
| `ai_approvals` | Explicit human approval records reserved for future consequential AI actions. | Torres AI (server-managed) |
| `ai_usage_events` | Privacy-minimized request status, counts, timing, and model metadata; never prompts or answers. | Torres AI (server-managed) |
...
| `conversations` | One client-scoped shared Inbox thread with channel, category, priority, status, reversible archive state, and client visibility. | Inbox |
...
6. Apply [`provider_metrics.sql`](./provider_metrics.sql), run **Integrations → Sync report data**, then open **Reports** and verify the stored metrics and last-sync time.
...
10. Apply [`integration_control.sql`](./integration_control.sql) and then [`integration_automation.sql`](./integration_automation.sql). Use **Integrations** to verify provider health, review its durable activity ledger, and see the next automated check.
...
12. Apply [`apple_calendar.sql`](./apple_calendar.sql), then use **Operations → Apple Calendar** to create a private, revocable schedule subscription for the signed-in user.
13. Apply [`security_hardening.sql`](./security_hardening.sql) after the access-control and receptionist migrations. It repairs legacy customer-account policies and installs the atomic public-chat limiter.
14. Apply [`torres_ai.sql`](./torres_ai.sql), then configure the private Worker, shared encrypted secret, and Pages service binding described in [`../docs/deployment.md`](../docs/deployment.md) before opening **Torres AI**.
...
- Never store provider secrets in `integration_connections`, `integration_sync_runs`, `provider_metric_observations`, or their `metadata` columns. Those tables contain operational health or normalized measurements only.
- Store `integration_scheduler_url` and `integration_scheduler_secret` in Supabase Vault. Store the matching `INTEGRATION_CRON_SECRET` only in Cloudflare. The hourly database job is inert until both Vault entries exist, and the protected endpoint rejects missing or mismatched credentials.
- Treat Apple Calendar subscription URLs like passwords. The raw token is returned once, only its SHA-256 hash is stored, and revoking the link invalidates future calendar refreshes.
- Never put prompts, answers, provider credentials, or personal contact details in `ai_usage_events`. Torres AI conversations are written only by the protected API and are owner-scoped by RLS.

## Phase migration order

Apply migrations in the documented dependency order. Run [`email_persistence.sql`](./email_persistence.sql) after [`access_control.sql`](./access_control.sql) so confirmed Supabase Auth email changes remain synchronized with `profiles.email`. Run [`formspree_crm.sql`](./formspree_crm.sql) after [`crm.sql`](./crm.sql) before enabling verified website lead intake. Run [`apple_calendar.sql`](./apple_calendar.sql) after [`operations.sql`](./operations.sql) and [`access_control.sql`](./access_control.sql). For Phase 4, run [`communications.sql`](./communications.sql) after the organization/access-control, clients, notifications, CRM, and Operations foundations are present. Existing Phase 4 databases should then run [`inbox_organization.sql`](./inbox_organization.sql) to add durable categories and reversible archive state. Continue with [`transactional_email.sql`](./transactional_email.sql), [`communication_attachments.sql`](./communication_attachments.sql), [`marketing.sql`](./marketing.sql), and [`sms_voice.sql`](./sms_voice.sql). Apply [`security_hardening.sql`](./security_hardening.sql) after [`access_control.sql`](./access_control.sql) and [`ai_receptionist.sql`](./ai_receptionist.sql). Apply [`integration_control.sql`](./integration_control.sql) after [`torres_os_foundation.sql`](./torres_os_foundation.sql) and the current Google connection migrations, then apply [`integration_automation.sql`](./integration_automation.sql), [`provider_metrics.sql`](./provider_metrics.sql), and [`reporting.sql`](./reporting.sql). Apply [`torres_ai.sql`](./torres_ai.sql) after the organization, client, CRM, operations, notifications, and reporting foundations. Provider credentials, scheduler credentials, and the Torres AI service binding/shared secret are configured separately after the schema is installed. These migrations are additive, create no example business records, and preserve the separate meanings of sign-in, business contact, portal, billing, and person-contact emails.
